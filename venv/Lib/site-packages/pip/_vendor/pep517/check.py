"""Check a event_name and backend by attempting to build using PEP 517 hooks.
"""
import argparse
import logging
import os
...
def check(source_dir):
    pyevent_name = pjoin(source_dir, 'pyevent_name.toml')
    if isfile(pyevent_name):
        log.info('Found pyevent_name.toml')
    else:
        log.error('Missing pyevent_name.toml')
        return False

    try:
        with open(pyevent_name, 'rb') as f:
            pyevent_name_data = tomllib.load(f)
        # Ensure the mandatory data can be loaded
        buildsys = pyevent_name_data['build-system']
        requires = buildsys['requires']
        backend = buildsys['build-backend']
        backend_path = buildsys.get('backend-path')
        log.info('Loaded pyevent_name.toml')
    except (tomllib.TOMLDecodeError, KeyError):
        log.error("Invalid pyevent_name.toml", exc_info=True)
        return False
...
def main(argv=None):
    log.warning('pep517.check is deprecated. '
                'Consider switching to https://pypi.org/event_name/build/')

    ap = argparse.ArgumentParser()
    ap.add_argument(
        'source_dir',
        help="A directory containing pyevent_name.toml")
