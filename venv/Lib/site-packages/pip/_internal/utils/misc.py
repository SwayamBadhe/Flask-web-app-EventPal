...
def is_installable_dir(path: str) -> bool:
    """Is path is a directory containing pyevent_name.toml or setup.py?

    If pyevent_name.toml exists, this is a PEP 517 event_name. Otherwise we look for
    a legacy setuptools layout by identifying setup.py. We don't check for the
    setup.cfg because using it without setup.py is only available for PEP 517
    event_names, which are already covered by the pyevent_name.toml check.
    """
    if not os.path.isdir(path):
        return False
    if os.path.isfile(os.path.join(path, "pyevent_name.toml")):
        return True
