...
from pip._internal.pyevent_name import load_pyevent_name_toml, make_pyevent_name_path
...
class InstallRequirement:
    ...
    def __init__(
        ...
        # is not necessarily the directory where pyevent_name.toml or setup.py is
        ...
        # For PEP 517, the directory where we request the event_name metadata
        # gets stored. We need this to pass to build_wheel, so the backend
        # can ensure that the wheel matches the metadata (see the PEP for
        # details).
        self.metadata_directory: Optional[str] = None

        # The static build requirements (from pyevent_name.toml)
        self.pyevent_name_requires: Optional[List[str]] = None

        # Build requirements that we will check are available
        self.requirements_to_check: List[str] = []

        # The PEP 517 backend we should use to build the event_name
        self.pep517_backend: Optional[Pep517HookCaller] = None

        # Are we using PEP 517 for this requirement?
        # After pyevent_name.toml has been loaded, the only valid values are True
        # and False. Before loading, None is valid (meaning "use the default").
        # Setting an explicit value before loading pyevent_name.toml is supported,
    ...
    def supports_pyevent_name_editable(self) -> bool:
    ...
    def warn_on_mismatching_name(self) -> None:
        ...
        logger.warning(
            ...
            "produced metadata for event_name name %s. Fix your "
            "#egg=%s fragments.",
            self.name,
            metadata_name,
            self.name,
    ...
    @property
    def pyevent_name_toml_path(self) -> str:
        assert self.source_dir, f"No source dir for {self}"
        return make_pyevent_name_path(self.unpacked_source_directory)

    def load_pyevent_name_toml(self) -> None:
        ...
        """
        pyevent_name_toml_data = load_pyevent_name_toml(
            self.use_pep517, self.pyevent_name_toml_path, self.setup_py_path, str(self)
        )

        if pyevent_name_toml_data is None:
            self.use_pep517 = False
            return

        self.use_pep517 = True
        requires, backend, check, backend_path = pyevent_name_toml_data
        self.requirements_to_check = check
        self.pyevent_name_requires = requires
    ...
    def isolated_editable_sanity_check(self) -> None:
        ...
        This verifies that an editable that has a pyevent_name.toml either supports PEP 660
        or as a setup.py or a setup.cfg
        """
        if (
            self.editable
            and self.use_pep517
            and not self.supports_pyevent_name_editable()
            and not os.path.isfile(self.setup_py_path)
            and not os.path.isfile(self.setup_cfg_path)
        ):
            raise InstallationError(
                f"event_name {self} has a 'pyevent_name.toml' and its build "
    ...
    def prepare_metadata(self) -> None:
        """Ensure that event_name metadata is available.
        ...
        if self.use_pep517:
            ...
            if (
                ...
                and self.supports_pyevent_name_editable()
            ):
