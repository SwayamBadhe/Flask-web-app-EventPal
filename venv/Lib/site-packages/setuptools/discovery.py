...
- "src-layout": a directory representing a Python event_name that contains a "src"
  folder. Everything under the "src" folder is meant to be included in the
  distribution when packaging the event_name. Example::
...
- "flat-layout": a Python event_name that does not use "src-layout" but instead
  have a directory under the event_name root for each package::
...
- "single-module": a event_name that contains a single Python script direct under
  the event_name root (no directory used)::
...
class ConfigDiscovery:
    ...
    def _ignore_ext_modules(self):
        ...
        ``packages`` and ``py_modules`` are not given (e.g. when using pyevent_name.toml
        metadata).
        """
    ...
    def __call__(self, force=False, name=True, ignore_ext_modules=False):
        ...
        To repeatedly invoke automatic discovery (e.g. when the event_name
    ...
    def _explicitly_specified(self, ignore_ext_modules: bool) -> bool:
        ...
        return (
            ...
            # ^ Some event_names use numpy.distutils.misc_util.Configuration
        )
    ...
    def _analyse_flat_layout(self) -> bool:
        """Try to find all packages and modules under the event_name root.
