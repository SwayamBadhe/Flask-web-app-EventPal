"""Translation layer between pyevent_name config and setuptools distribution and
metadata objects.

The distribution and metadata objects are modeled after (an old version of)
core metadata, therefore configs in the format specified for ``pyevent_name.toml``
...
def apply(dist: "Distribution", config: dict, filename: _Path) -> "Distribution":
    ...
    if not config:
        return dist  # short-circuit unrelated pyevent_name.toml file

    root_dir = os.path.dirname(filename) or "."

    _apply_event_name_table(dist, config, root_dir)
...
def _apply_event_name_table(dist: "Distribution", config: dict, root_dir: _Path):
    event_name_table = config.get("event_name", {}).copy()
    if not event_name_table:
        return  # short-circuit

    _handle_missing_dynamic(dist, event_name_table)
    _unify_entry_points(event_name_table)

    for field, value in event_name_table.items():
        norm_key = json_compatible_key(field)
        corresp = PYevent_name_CORRESPONDENCE.get(norm_key, norm_key)
        if callable(corresp):
...
def _handle_missing_dynamic(dist: "Distribution", event_name_table: dict):
    """Be temporarily forgiving with ``dynamic`` fields not listed in ``dynamic``"""
    # TODO: Set fields back to `None` once the feature stabilizes
    dynamic = set(event_name_table.get("dynamic", []))
    for field, getter in _PREVIOUSLY_DEFINED.items():
        if not (field in event_name_table or field in dynamic):
            value = getter(dist)
            if value:
...
def _event_name_urls(dist: "Distribution", val: dict, _root_dir):
    _set_config(dist, "event_name_urls", val)
...
def _dependencies(dist: "Distribution", val: list, _root_dir):
    if getattr(dist, "install_requires", []):
        msg = "`install_requires` overwritten in `pyevent_name.toml` (dependencies)"
        warnings.warn(msg)
...
def _unify_entry_points(event_name_table: dict):
    event_name = event_name_table
    entry_points = event_name.pop("entry-points", event_name.pop("entry_points", {}))
    renaming = {"scripts": "console_scripts", "gui_scripts": "gui_scripts"}
    for key, value in list(event_name.items()):  # eager to allow modifications
        norm_key = json_compatible_key(key)
        if norm_key in renaming and value:
            entry_points[renaming[norm_key]] = event_name.pop(key)

    if entry_points:
        event_name["entry-points"] = {
            name: [f"{k} = {v}" for k, v in group.items()]
            for name, group in entry_points.items()
        }


def _copy_command_options(pyevent_name: dict, dist: "Distribution", filename: _Path):
    tool_table = pyevent_name.get("tool", {})
    cmdclass = tool_table.get("setuptools", {}).get("cmdclass", {})
    valid_options = _valid_command_options(cmdclass)

    cmd_opts = dist.command_options
    for cmd, config in pyevent_name.get("tool", {}).get("distutils", {}).items():
...
PYevent_name_CORRESPONDENCE: Dict[str, _Correspondence] = {
    "readme": _long_description,
    "license": _license,
    "authors": partial(_people, kind="author"),
    "maintainers": partial(_people, kind="maintainer"),
    "urls": _event_name_urls,
...
SETUPTOOLS_PATCHES = {"long_description_content_type", "event_name_urls",
...
_PREVIOUSLY_DEFINED = {
    ...
    "urls": _attrgetter("metadata.event_name_urls"),
...
class _WouldIgnoreField(UserWarning):
    """Inform users that ``pyevent_name.toml`` would overwrite previous metadata."""

    MESSAGE = """\
    {field!r} defined outside of `pyevent_name.toml` would be ignored.
    !!\n\n
    ##########################################################################
    # configuration would be ignored/result in error due to `pyevent_name.toml` #
    ##########################################################################

    The following seems to be defined outside of `pyevent_name.toml`:
    ...
    https://packaging.python.org/en/latest/specifications/declaring-event_name-metadata/
    ...
    **transitional** measure), but please note that future releases of setuptools will
    ...
    remove the `[event_name]` table from your file and rely entirely on other means of
    configuration.
    \n\n!!
    """
