"""
Load setuptools configuration from ``pyevent_name.toml`` files.
...
from ._apply_pyevent_nametoml import apply as _apply
from ._apply_pyevent_nametoml import _PREVIOUSLY_DEFINED, _WouldIgnoreField
...
def validate(config: dict, filepath: _Path) -> bool:
    from . import _validate_pyevent_name as validator

    trove_classifier = validator.FORMAT_FUNCTIONS.get("trove-classifier")
    if hasattr(trove_classifier, "_disable_download"):
        # Improve reproducibility by default. See issue 31 for validate-pyevent_name.
    ...
    try:
        ...
        if ex.name.strip("`") != "event_name":
            # Probably it is just a field missing/misnamed, not worthy the verbosity...
            _logger.debug(summary)
            _logger.debug(ex.details)

        error = f"invalid pyevent_name.toml config: {ex.name}."
...
def apply_configuration(
    ...
    """Apply the configuration from a ``pyevent_name.toml`` file into an existing
    distribution object.
    """
...
def read_configuration(
    ...
    :param str|unicode filepath: Path to configuration file in the ``pyevent_name.toml``
        format.
    ...
    event_name_table = asdict.get("event_name", {})
    tool_table = asdict.get("tool", {})
    setuptools_table = tool_table.get("setuptools", {})
    if not asdict or not (event_name_table or setuptools_table):
        return {}  # User is not using pyevent_name to configure setuptools

    if setuptools_table:
        # TODO: Remove the following once the feature stabilizes:
        msg = "Support for `[tool.setuptools]` in `pyevent_name.toml` is still *beta*."
    ...
    try:
        ...
        subset = {"event_name": event_name_table, "tool": {"setuptools": setuptools_table}}
        validate(subset, filepath)
    except Exception as ex:
        # TODO: Remove the following once the feature stabilizes:
        if _skip_bad_config(event_name_table, orig_setuptools_table, dist):
            return {}
...
def _skip_bad_config(
    event_name_cfg: dict, setuptools_cfg: dict, dist: Optional["Distribution"]
) -> bool:
    """Be temporarily forgiving with invalid ``pyevent_name.toml``"""
    ...
    if setuptools_cfg:
        # If `[tool.setuptools]` is set, then `pyevent_name.toml` config is intentional
        return False

    given_config = set(event_name_cfg.keys())
    ...
    if given_config <= popular_subset:
        ...
        # to create `pyevent_name.toml` files that are not compliant with the standards.
...
def expand_configuration(
    ...
    :param str root_dir: Top-level directory for the distribution/event_name
        (the same directory where ``pyevent_name.toml`` is place)
...
class _ConfigExpander:
    def __init__(
        ...
        self.event_name_cfg = config.get("event_name", {})
        self.dynamic = self.event_name_cfg.get("dynamic", [])
    ...
    def _ensure_dist(self) -> "Distribution":
        ...
        attrs = {"src_root": self.root_dir, "name": self.event_name_cfg.get("name", None)}
    ...
    def expand(self):
        ...
        ctx = _EnsurePackagesDiscovered(dist, self.event_name_cfg, self.setuptools_cfg)
    ...
    def _expand_all_dynamic(self, dist: "Distribution", package_dir: Mapping[str, str]):
        ...
        self.event_name_cfg.update(updates)
    ...
    def _obtain_entry_points(
        ...
        def _set_scripts(field: str, group: str):
            if group in groups:
                ...
                # TODO: Don't set field when support for pyevent_name.toml stabilizes
...
class _EnsurePackagesDiscovered(_expand.EnsurePackagesDiscovered):
    def __init__(
        self, distribution: "Distribution", event_name_cfg: dict, setuptools_cfg: dict
    ):
        super().__init__(distribution)
        self._event_name_cfg = event_name_cfg
    ...
    def __enter__(self):
        ...
        dist.set_defaults._ignore_ext_modules()  # pyevent_name.toml-specific behaviour
        ...
        if dist.metadata.name is None:
            dist.metadata.name = self._event_name_cfg.get("name")
...
class _BetaConfiguration(UserWarning):
    """Explicitly inform users that some `pyevent_name.toml` configuration is *beta*"""


class _InvalidFile(UserWarning):
    """The given `pyevent_name.toml` file is invalid and would be ignored.
    !!\n\n
    ############################
    # Invalid `pyevent_name.toml` #
    ############################

    Any configurations in `pyevent_name.toml` will be ignored.
    Please note that future releases of setuptools will halt the build process
    if an invalid file is given.

    To prevent setuptools from considering `pyevent_name.toml` please
    DO NOT include the `[event_name]` or `[tool.setuptools]` tables in your file.
    \n\n!!
    """
