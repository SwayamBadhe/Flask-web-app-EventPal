...
from setuptools.config import setupcfg, pyevent_nametoml
...
def write_pkg_file(self, file):  # noqa: C901  # is too complex (14)  # FIXME
    ...
    for event_name_url in self.event_name_urls.items():
        write_field('event_name-URL', '%s, %s' % event_name_url)
...
def check_extras(dist, attr, value):
    ...
    try:
        ...
        raise DistutilsSetupError(
            ...
            "strings or lists of strings containing valid event_name/version "
...
def check_requirements(dist, attr, value):
    ...
    try:
        ...
        tmpl = (
            ...
            "containing valid event_name/version requirement specifiers; {error}"
        )
...
class Distribution(_Distribution):
    ...
    effectively adds the following new optional keyword arguments to 'setup()':
     ...
     'install_requires' -- a string or sequence of strings specifying event_name
        versions that the distribution requires when installed, in the format
        used by 'pkg_resources.require()'.  They will be installed
        automatically when the package is installed.  If you wish to use
        packages that are not available in PyPI, or want to give your users an
        alternate download location, you can add a 'find_links' option to the
        '[easy_install]' section of your event_name's 'setup.cfg' file, and then
    ...
    def _get_event_name_config_files(self, filenames):
        """Add default file and split between INI and TOML"""
        tomlfiles = []
        standard_event_name_metadata = Path(self.src_root or os.curdir, "pyevent_name.toml")
        if filenames is not None:
            parts = partition(lambda f: Path(f).suffix == ".toml", filenames)
            filenames = list(parts[0])  # 1st element => predicate is False
            tomlfiles = list(parts[1])  # 2nd element => predicate is True
        elif standard_event_name_metadata.exists():
            tomlfiles = [standard_event_name_metadata]
    ...
    def parse_config_files(self, filenames=None, ignore_option_errors=False):
        ...
        """
        inifiles, tomlfiles = self._get_event_name_config_files(filenames)
        ...
        for filename in tomlfiles:
            pyevent_nametoml.apply_configuration(self, filename, ignore_option_errors)
