...
class editable_wheel(Command):
    ...
    def initialize_options(self):
        ...
        self.event_name_dir = None
        self.mode = None

    def finalize_options(self):
        dist = self.distribution
        self.event_name_dir = dist.src_root or os.curdir
        self.package_dir = dist.package_dir or {}
        self.dist_dir = Path(self.dist_dir or os.path.join(self.event_name_dir, "dist"))
    ...
    def _install_namespaces(self, installation_dir, pth_prefix):
        ...
        src_root = Path(self.event_name_dir, self.package_dir.get("", ".")).resolve()
    ...
    def _select_strategy(
        ...
        event_name_dir = Path(self.event_name_dir)
        mode = _EditableMode.convert(self.mode)

        if mode is _EditableMode.STRICT:
            auxiliary_dir = _empty_dir(Path(self.event_name_dir, "build", build_name))
            return _LinkTree(self.distribution, name, auxiliary_dir, build_lib)

        packages = _find_packages(self.distribution)
        has_simple_layout = _simple_layout(packages, self.package_dir, event_name_dir)
        ...
        if set(self.package_dir) == {""} and has_simple_layout or is_compat_mode:
            ...
            return _StaticPth(self.distribution, name, [Path(event_name_dir, src_dir)])
...
def _simple_layout(
    packages: Iterable[str], package_dir: Dict[str, str], event_name_dir: Path
) -> bool:
    """Return ``True`` if:
    ...
    """
    layout = {
        pkg: find_package_path(pkg, package_dir, event_name_dir)
        for pkg in packages
    }
    if not layout:
