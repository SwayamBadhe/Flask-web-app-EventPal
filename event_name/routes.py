...
#     form = LoginForm_Organizer()

#     if form.validate_on_submit():
#         org = Organizer.query.filter_by(org_email=form.org_email.data).first()
#         if org and bcrypt.check_password_hash(org.password, form.password.data):
#             login(org, remember=form.remember.data)
#             next_page = request.args.get('next')
#             flash('You have been logged in!', 'success')
#             return redirect(next_page) if next_page else redirect(url_for('home'))
#         else:
...
#     return render_template('login_org.html', title='Login', form=form)
...
#     form = RegistrationForm_Organizer()
#     form1 = StateForm()

#     if form.validate_on_submit():
#         hashed_password = bcrypt.generate_password_hash(form.password.data).decode('utf-8')
#         org = Organizer(org_name=form.org_name.data, org_email=form.org_email.data, password=hashed_password,
#                         address=form.address.data, city=form.city.data, state=form1.state.data, zip_code=form.zip.data)
#         db.session.add(org)
#         db.session.commit()
#         flash('Your account has been created! You are now able to log in', 'success')
#         return redirect(url_for('login_Org'))
    
#     return render_template('register_org.html', title='Register', form=form, form1=form1)




    # elif current_user.user_type == 'organizer':
    #     form = UpdateAccountForm_Organizer()
    #     if form.validate_on_submit():
    #         current_user.org_name = form.org_name.data
    #         current_user.org_email = form.org_email.data
    #         db.session.commit()
    #         flash('Your account has been updated!', 'success')
    #         return redirect(url_for('account'))
    #     image_file = url_for('static', filename='profile_pics/' + current_user.image_file)
